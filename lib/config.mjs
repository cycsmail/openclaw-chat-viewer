...
function resolveSyncInterval(configured) {
  if (!configured) return 0;
  const parsed = Number(configured);
  if (Number.isInteger(parsed) && parsed >= 1 && parsed <= 7 * 24 * 60) return parsed;
  return 0;
}
...
export function initConfig() {
  ...
  _cfg.USERS_PATH = path.join(_cfg.ARCHIVE_DIR, "users.json");
  _cfg.HOST = process.env.HOST || "0.0.0.0";
  ...
  _cfg.MACHINES_PATH = path.join(_cfg.ARCHIVE_DIR, "machines.json");
  _cfg.MACHINE_CACHE_DIR = path.join(_cfg.ARCHIVE_DIR, "machines");
  _cfg.SYNC_INTERVAL_MINUTES = resolveSyncInterval(process.env.OPENCLAW_SYNC_INTERVAL_MINUTES);
  _cfg.SYNC_INTERVAL_MS = _cfg.SYNC_INTERVAL_MINUTES > 0 ? _cfg.SYNC_INTERVAL_MINUTES * 60_000 : 0;
...
export function getMachinesPath() { return _cfg.MACHINES_PATH; }
export function getMachineCacheDir() { return _cfg.MACHINE_CACHE_DIR; }
export function getSyncIntervalMs() { return _cfg.SYNC_INTERVAL_MS; }
export function getUsersPath() { return _cfg.USERS_PATH; }
...
export const KNOWN_ERRORS = new Map([
  ...
  ["Invalid machine profile", 400],
  ["Machine not found", 404],
  ["Invalid machineId", 400],
