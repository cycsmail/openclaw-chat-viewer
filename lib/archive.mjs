...
export function getSearchIndexPath() {
  return path.join(getArchiveDir(), SEARCH_INDEX_FILENAME);
}

export function createSearchIndex() {
  return { tokens: {}, snapshotMeta: {} };
}

export async function loadSearchIndex() {
  const data = await readJson(getSearchIndexPath(), null);
  if (!data || typeof data !== "object") {
    return createSearchIndex();
  }
  return {
    tokens: data.tokens && typeof data.tokens === "object" ? data.tokens : {},
    snapshotMeta: data.snapshotMeta && typeof data.snapshotMeta === "object" ? data.snapshotMeta : {}
  };
}

export async function saveSearchIndex(index) {
  const archiveDir = getArchiveDir();
...
export function extractSearchTokens(text) {
  ...
  const raw = text.toLowerCase().split(/[\s\W]+/).filter((t) => t.length >= 2);
  return [...new Set(raw)];
}
